...
### We will do this together in class. Not graded...just a chance to walk through an exam so the real thing goes smoothly.

_____________

# SKILLS TEST 1 (practice)
_____________
...
The files you just got from your "git pull" are:
 ...
 + README.md (this file containing the test prompts)
 + README.html (pretty version of the md file)
...
# YOUR TASKS 
 ...
 + You MUST use an R-project called "Exam_1_Practice.Rproj" so all your file paths will start from your new Exam_1 directory!
...
One way to do this is a separate boxplot for each student who did extractions. Show the boxplot for each "Year_Collected"

III.
Once you have your code for creating the figures correctly, you need to save those two images in YOUR Exam_1 directory. Name them Katy_Extractions_boxplot.png and Ben_Extractions_boxplot.png, respectively
...
V.
Create a subset of the data for me. Subset the data frame so it's just the "Downstairs" lab.
Now, make a scatterplot of the downstairs lab data such that "Date_Collected" is on the x-axis and "DNA_Concentration_Ben" is on the y-axis. Save this scatterplot as "Ben_DNA_over_time.png" in your Exam_1 directory. 

You might need to do some class conversions so the plot() function treats things correctly. HintHintHint: POSIXct
...
and another column that contains the AVERAGE of the values within that year.  Just to be clear, this data frame should have only 12 rows (one for each year)! You will need to find a way to take the average of Ben's DNA values in each separate year. 
...
10 pts - Make boxplots by year for each lab tech's concentrations (proper labels)
...
20 pts - Code to produce a scatterplot of Ben_DNA_over_time.png showing just the "Downstairs" data.
